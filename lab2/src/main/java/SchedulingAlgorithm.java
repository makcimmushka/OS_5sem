...
public class SchedulingAlgorithm {
    ...
    private enum ProcessStatesEnum {
        REGISTERED,
        COMPLETED,
        JOINED,
        FORCEDOUT
    ...
    private static void print(int time, PrintStream out, ProcessStatesEnum state,
                              sProcess process, Vector<sProcess> processVector) {
        print(time, out, state, process, processVector, false);
    }

    private static void print(int time, PrintStream out, ProcessStatesEnum state, sProcess process,
                              Vector<sProcess> processVector, boolean quantumExpired) {
        out.println(time + "ms ");
            ...
            while (comptime < runtime) {
                ...
                if (lastProcessIndex < size) {
                    while (sortedProcessVector.elementAt(lastProcessIndex).delay == comptime) {
                        ...
                        print(comptime, out, ProcessStatesEnum.JOINED, processCopy, processVector);
                        ++lastProcessIndex;
                        if (lastProcessIndex >= size) {
                            break;
                        }
                ...
                if (process == null && !queue.isEmpty()) {
                    process = queue.peek();
                    print(comptime, out, ProcessStatesEnum.REGISTERED, process, processVector);
                }

                if (process != null && process.cpuDone == process.cpuTime) {
                    completed++;

                    print(comptime, out, ProcessStatesEnum.COMPLETED, process, processVector, quantumCounter == quantum);
                ...
                if (quantumCounter == quantum) {
                    ...
                    print(comptime, out, ProcessStatesEnum.FORCEDOUT, process, processVector, true);
